...
<div class="panel panel-primary">
	...
	<div class="panel-body">
		...
		<p>
			<ol>
				<!--
				<li><b><?php _e('Keep Existing Files','memphis-documents-library'); ?></b>
				...
				-->
				<li><b><?php _e('Delete Old Files','memphis-documents-library'); ?></b>
		...
		<form  id="mdocs-import" method="post" action="admin.php?page=memphis-documents.php&mdocs-cat=import" enctype="multipart/form-data">
			...
			<p><b><i><?php _e('Remember to always export any valuable data before doing an import.','memphis-documents-library'); ?></i></b></p>
			...
			<input type="submit" class="button button-primary" id="mdocs-import-submit" value="<?php _e('Import Memphis Documents Library','memphis-documents-library') ?>" />
		</form>
	</div>
</div>
<?php
				...
				foreach($mdocs_list_file as $key => $value) {
					...
					if($valid_mime_type) {
						...
						if(file_exists($zip_result['dir'].'/'.$value['filename'])) {
							rename($zip_result['dir'].'/'.$value['filename'], $upload_dir['basedir'].'/mdocs/'.strtolower($value['filename']));
						} else $file_not_found = true;
						if(is_array($value['archived']) && count($value['archived']) > 0){
							foreach($value['archived'] as $archive) {
								if(file_exists($zip_result['dir'].'/'.$archive)) rename($zip_result['dir'].'/'.$archive, $upload_dir['basedir'].'/mdocs/'.strtolower($archive));
							}
						}
						...
						elseif($hide_all_posts == '1') $the_post_stauts = 'publish';
						...
						elseif($hide_all_posts == '1') $the_post_stauts_sys = 'publish';
