...
function mdocs_edit_file($the_mdocs, $index, $current_cat) {
	...
	<div class="mdocs-edit-file">
		<span class="update" id="<?php echo $index ?>">
			<i class="fa fa-pencil"></i> <a href="<?php echo 'admin.php?page=memphis-documents.php&mdocs-cat='.$current_cat.'&action=update-doc&mdocs-index='.$index; ?>" title="Update this file" class="edit"><?php _e('Update','memphis-documents-library'); ?></a> |
		</span>
		<span class='delete'>
			<i class="fa fa-remove"></i> <a class='submitdelete' onclick="return showNotice.warn();" href="<?php echo 'admin.php?mdocs-nonce='.$_SESSION['mdocs-nonce'].'&page=memphis-documents.php&mdocs-cat='.$current_cat.'&action=delete-doc&mdocs-index='.$index; ?>"><?php _e('Delete','memphis-documents-library'); ?></a> |
		</span>
		<span class="versions">
 ...
 * @param float $time_offset If sending a time that is not UTC
 * @return array The date array.
 */
function mdocs_format_date($date, $time_offset=0) {
	...
	if(method_exists('DateTime', 'createFromFormat')) {
		...
		if($dtime != false) {
			$gmdate = $formated_dates['gmdate'] = floatval($dtime->getTimestamp()-$time_offset);
...
function mdocs_process_file($file, $import=false) {
	...
	if($import == false && isset($_POST['mdocs-last-modified'])) {
		$the_date = mdocs_format_date($_POST['mdocs-last-modified'], MDOCS_TIME_OFFSET);
	...
	if($import == false) {
		...
		if($upload_result == false) $upload['error'] = __('An Error has occured on line 320 of the mdocs-functions.php file.','memphis-documents-library');
	} else {
...
function mdocs_nonce() {
	if(get_option('mdocs-disable-sessions') == false && function_exists('session_status')) {
		$mdocs_init_request = md5(microtime());
		if(!isset($_COOKIE['mdocs-id'])) {
			if(is_ssl()) $the_cookie = setcookie('mdocs-id', $mdocs_init_request, 0, '/', '', true, true);
			else $the_cookie = setcookie('mdocs-id', $mdocs_init_request, 0, '', '', false, true);
			$mdocs_id = md5($mdocs_init_request);
		} else $mdocs_id = md5($_COOKIE['mdocs-id']);
		session_name('mdocs-session');
		if(is_ssl()) session_set_cookie_params(0, '/', '', true, true);
		else session_set_cookie_params(0, '/', '', false, true);
		session_id($mdocs_id);
		if(session_status() == 1) session_start();
		if(!isset($_SESSION['mdocs-id'])) $_SESSION['mdocs-id'] = $mdocs_id;
		...
		$mdocs_session_id = $_SESSION['mdocs-id'];
		session_write_close();
...
function mdocs_errors($error, $type='updated') {
	if($type == 'error') $error = '<b>'.__('Memphis Error','memphis-documents-library').': </b>'.$error;
	else $error = '<b>'.__('Memphis Info','memphis-documents-library').': </b>'.$error;
	?>
...
function mdocs_get_the_folder($current_folder=null, $shortcode=null) {
	...
	elseif(isset($_GET['att']) && $shortcode != $_GET['att']) $current_folder = $shortcode;
...
function mdocs_get_permalink($id, $override_admin=false) {
	if(!is_admin() || $override_admin) {
		$permalink = mdocs_sanitize_string(get_permalink($id));
...
function mdocs_get_num_cols() {
	...
	if(get_option('mdocs-show-download-btn') == '1' || get_option('mdocs-show-download-btn')) $num_cols++;
...
function mdocs_get_parents($child, $permalink='', $att=null, $tabs = array('parent-tab' => '', 'current-tab' => '&emsp;&emsp;', 'child-tab' => '&emsp;&emsp;&emsp;&emsp;')) {
	...
	$emsp = '&emsp;&emsp;';
	if($parent['parent'] != '') {
		mdocs_get_parents($parent['parent'], $permalink, $att, $tabs);
	...
	$html = '
			<tr class="mdocs-parent-cat" >
				<td colspan="'.mdocs_get_num_cols().'" id="title" class="mdocs-tooltip">
					'.$tabs['parent-tab'].'<a href="'.$permalink.$parent['slug'].'&att='.$att.'" title="'.$permalink.$parent['slug'].'"><i class="fa fa-folder-o"></i> '.$parent['name'].'</a>
				</td>
			</tr>';
	echo $html;
 ...
 */
function mdocs_display_sub_folders($current_cat, $att=null) {
	global $post;
	...
	if($current_cat['parent'] != '') {
		$tabs = mdocs_get_parents($current_cat['parent'], $permalink, $att);	
	} else {
	...
	$current_cat_string = '
		<tr class="mdocs-current-cat" >
			<td colspan="'.$num_cols.'" id="title" class="mdocs-tooltip">
				'.$tabs['current-tab'].'<i class="fa fa fa-folder-open-o"></i> '.$current_cat['name'].'
			</td>
		</tr>';
	if(isset($current_cat['children']) && count($current_cat['children']) > 0) {
		foreach($current_cat['children'] as $index => $child) {
			...
			<tr class="mdocs-sub-cats" >
				<td colspan="'.$num_cols.'" id="title" class="mdocs-tooltip">
					'.$tabs['child-tab'].'<a href="'.$permalink.$child['slug'].'&att='.$att.'" title="'.$child['name'].'"><i class="fa fa-folder-o"></i> '.$child['name'].'</a>
				</td>
			</tr>';
	...
	if($show) {
		...
		<div class="mdocs-wrap">
			...
			<div class="btn-group" role="group">
				<button class="add-update-btn btn btn-danger" data-toggle="modal" data-target="#mdocs-add-update" data-mdocs-id="" data-is-admin="<?php echo is_admin(); ?>" data-action-type="add-doc"  data-current-cat="<?php echo $current_cat; ?>" href=""><i class="fa fa-upload fa-lg"></i> <?php _e('Add New Document','memphis-documents-library'); ?></button>
				<button class="btn btn-default" onclick="mdocs_batch_edit()" data-toggle="mdocs-modal" data-target="#mdocs-batch-edit"><i class="fa fa-pencil" aria-hidden="true"></i> <?php _e('Batch Edit', 'memphis-documents-library'); ?></button>
...
function mdocs_get_current_cat($atts=null) {
	...
	elseif(isset($_GET['mdocs-cat']) && isset($atts['cat']) && isset($_GET['att']) &&  $atts['cat'] == mdocs_sanitize_string($_GET['att'])) {
...
if(get_option('mdocs-show-advanced-search') == true) add_filter( 'posts_where', 'mdocs_robust_search' );
...
function mdocs_box_view_update_v3_0() {
	...
	<div class="container-3-0">
		...
		<p><b><?php _e('The process depending on the size of your Library can take a long time, so make sure you have the time run this updater.', 'memphis-documents-library'); ?></b></p>
		<p><b><?php _e('If you choose not to run this updater now preview will not work.', 'memphis-documents-library'); ?></b></p>
		<p><b><?php _e('You may run this process anytime by going to the Settings menu and pressing "Run Preview and Thumbnail Updater".', 'memphis-documents-library'); ?></b></p>
