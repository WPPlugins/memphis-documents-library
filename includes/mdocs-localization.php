...
function mdocs_ajax_processing() {
	if(check_ajax_referer( 'mdocs-ajax-nonce', '_mdocs_ajax_nonce',false)) {
		...
		wp_die(__('mDocs Error: Memphis has terminated due to a faulty nonce check.  This check is needed to deter Cross-site scripting security vulnerabilities.', 'memphis-documents-library'));
	}
	exit;
}
function mdocs_localized_errors() {
	...
	define('MDOCS_ERROR_4',__('Session data is not formatted correct, Memphis Documents Library has disabled session data for you.  Please try again.','memphis-documents-library'));
