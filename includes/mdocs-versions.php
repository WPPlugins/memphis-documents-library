				...
				<table  class="wp-list-table widefat plugins">
					...
					<tbody id="the-list">
							<tr class="mdocs-bg-odd">
								...
								<td id="download"><input type="button" id="mdocs-download" onclick="mdocs_download_current_version('<?php echo $the_mdoc['id']; ?>')" class="button button-primary" value=<?php _e("Download", 'memphis-documents-library'); ?>  /></td>
								<td></td>
						...
						foreach( array_reverse($the_mdoc['archived']) as $key => $archive ){
							...
							if(file_exists($upload_dir['basedir'].'/mdocs/'.$archive) || file_exists($upload_dir['basedir'].'/mdocs/'.strtolower($archive))) {
								...
								<tr class="<?php echo $bgcolor; ?>">
									...
									<td id="download"><input onclick="mdocs_download_version('<?php echo $the_mdoc['id']; ?>', '<?php echo $archive; ?>')" type="button" id="mdocs-download" name="<?php echo $key; ?>" class="button button-primary" value=<?php _e("Download"); ?>  /></td>
...
function mdocs_delete_version() {
	if ($_GET['mdocs-nonce'] == MDOCS_NONCE || get_option('mdocs-disable-sessions') == true) {
		...
		mdocs_errors(MDOCS_ERROR_4,'error');
		update_option('mdocs-disable-sessions', true);
	}
}
